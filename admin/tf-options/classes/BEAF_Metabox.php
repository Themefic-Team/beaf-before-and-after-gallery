...
//                die();
			...
			if ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == "tf_hotel" ) {
				$tf_metabox_request = ( ! empty( $_POST['tf_hotels_opt'] ) && is_array( $_POST['tf_hotels_opt'] ) ) ? array_map( 'sanitize_text_field', wp_unslash( $_POST['tf_hotels_opt'] ) ) : array();
			}
			if ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == "tf_tours" ) {
				$tf_metabox_request = ( ! empty( $_POST['tf_tours_opt'] ) && is_array( $_POST['tf_tours_opt'] ) ) ? array_map( 'sanitize_text_field', wp_unslash( $_POST['tf_tours_opt'] ) ) : array();
			}
			if ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == "tf_apartment" ) {
				$tf_metabox_request = ( ! empty( $_POST['tf_apartment_opt'] ) && is_array( $_POST['tf_apartment_opt'] ) ) ? array_map( 'sanitize_text_field', wp_unslash( $_POST['tf_apartment_opt'] ) ) : array();
			}
			...
			if ( function_exists( 'is_tf_pro' ) && is_tf_pro() ) {
				if ( ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == "tf_hotel" ) || ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == "tf_tours" ) || ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == "tf_apartment" ) ) {
					do_action( 'bafg_services_pabbly_form_trigger', $post_id, $post_basic_info, $tf_metabox_request );
					do_action( 'bafg_services_zapier_form_trigger', $post_id, $post_basic_info, $tf_metabox_request );
				}
			}
		}
