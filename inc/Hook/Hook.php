...
class Hook {
    ...
    public function init(){
		...
		*/
		...
		add_action( 'wp_enqueue_scripts', array( new LoadAssest , 'bafg_image_before_after_foucs_scripts' ), 999 );

		// Check if Elementor installed and activated
		if ( did_action( 'elementor/loaded' ) ) {
			add_action( 'elementor/editor/before_enqueue_scripts', array( new LoadAssest, 'bafg_image_before_after_foucs_scripts' ) );
		}
		...
		*/
		...
		//enqueue scripts
		add_action( 'admin_enqueue_scripts', [ new LoadAssest, 'BEAF_tourfic_admin_denqueue_script' ], 20 );
    }
